...
class C(BaseConstants):
    ...
    base_payment = cu(25)
    multiplier = 90 ## A multiplier used in the calculation of participants' payoffs based on their trading performance
    min_payment_in_round = cu(0) ## The minimum payment that a participant can receive in a single round.
    min_payment = cu(4) ## The minimum total payment that a participant can receive across all rounds.
    FV_MIN = 30 ##  The minimum fundamental value of the asset being traded.
    FV_MAX = 85 ## The maximum fundamental value of the asset being traded.
    num_assets_MIN = 20 ## The minimum number of assets a participant can be endowed with.
    num_assets_MAX = 35 ## The maximum number of assets a participant can be endowed with.
    decimals = 2 ## The number of decimal places for prices in the experiment.
    marketTime = 210  ## The default duration of the market in seconds. This value is used to initialize variables but can be overridden by the session configuration.


class Subsession(BaseSubsession):
    offerID = models.IntegerField(initial=0) #  bid or ask made by a participant in the market.
    orderID = models.IntegerField(initial=0) # the action taken by a participant to either place an offer (limit or market) or cancel an existing offer
    transactionID = models.IntegerField(initial=0) # when a buy offer is matched with a sell offer, resulting in the actual exchange of assets and cash
...
def vars_for_admin_report(subsession):
    ...
    payoffs = sorted([p.payoff for p in subsession.get_players()]) # Calculates and sorts the payoffs of all players in the current subsession.
    market_times = sorted([g.marketTime for g in groups])
    highcharts_series = [] # Initializes an empty list to store series data for Highcharts, a JavaScript charting library.
...
class Group(BaseGroup):
    ...
    marketStartTime = models.FloatField() #  The timestamp (in seconds) when the market starts
    marketEndTime = models.FloatField() #  The timestamp (in seconds) when the market ends.
    randomisedTypes = models.BooleanField() # whether participant roles are randomized in this group.
    numAssets = models.IntegerField(initial=0) # The total number of assets held by all participants in the group.
    numParticipants = models.IntegerField(initial=0)
    numActiveParticipants = models.IntegerField(initial=0)
    assetNames = models.LongStringField() # A string field to store the names or identifiers of the assets being traded.
...
class Player(BasePlayer):
    ...
    selectedRound = models.IntegerField(initial=1) # The round number selected for final payoff calculation.
...
def set_player(player: Player):
    ...
    role_id = player.field_maybe_none('roleID')
    if player.isParticipating:
        player.isObserver = role_id == 'observer'
...
class Limit(ExtraModel):
    ...
    amount = models.FloatField(decimal=C.decimals) ##  the total value of a trade, calculated as the product of the price per unit of the asset and the number of units traded.
...
class WaitToStart(WaitPage):
    template_name = 'singleAsset/WaitToStart.html'
